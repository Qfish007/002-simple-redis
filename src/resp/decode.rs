...
impl RespDecode for SimpleString {
    ...
    fn decode(buf: &mut BytesMut) -> Result<Self, RespError> {
        ...
        // split_to 会从buf中切出前end + CRLF_LEN个字节，并返回切出的部分，剩余的部分会保留在buf中
        // println!("-----x1-- {:?} , {:?} , {:?}", end, buf, CRLF_LEN);
        let data: BytesMut = buf.split_to(end + CRLF_LEN);
...
fn extract_simple_frame_data(buf: &[u8], prefix: &str) -> Result<usize, RespError> {
    // 如果buf长度小于3，说明至少需要3个字节，一个是prefix，一个是CRLF，一个是\r
    if buf.len() < 3 {
        return Err(RespError::NotComplete);
    }

    // 如果buf不以prefix开头，说明不是一个合法的frame
    ...
    // 找到第一个CRLF，CRLF之前的内容就是frame的内容
    let end = find_crlf(buf, 1).ok_or(RespError::NotComplete)?;

    Ok(end)
}

// 查找第1个CRLF，CRLF之前的内容就是frame的内容
/*
    CRLF 是 "Carriage Return Line Feed" 的缩写，中文意思是回车换行符，由两个字符组成：

    \r (Carriage Return，回车符)
    \n (Line Feed，换行符)
*/
fn find_crlf(buf: &[u8], nth: usize) -> Option<usize> {
    // 从第1个字节开始找，因为第0个字节是prefix
    //  "+OK\r\n"
    // println!("find_crlf: buf: {:?}, nth: {}", buf, nth);
    ...
    /*
      +OK\r\n
      [43, 79, 75, 13, 10]
      find_crlf: buf: [43, 79, 75, 13, 10], nth: 1

      1..buf.len() - 1 是 1到4的范围，分别是79, 75, 13, 10
    */
...
fn parse_length(buf: &[u8], prefix: &str) -> Result<(usize, usize), RespError> {
    ...
    // 从prefix后面开始，到end之前，就是数据
    let s = String::from_utf8_lossy(&buf[prefix.len()..end]);
    /*
        假设 buf 是 b"$5\r\nhello\r\n" ， prefix 是 "$" ：
        1. extract_simple_frame_data 会找到 \r\n 的位置，假设返回 2
        2. s 会是 "5" （从索引 1 到 2）
        3. s.parse()? 会将 "5" 解析为 usize 值 5
        4. 函数返回 Ok((2, 5))

        parse_length 就是要找到数据的结束位位置，和 数据的长度 如数组长度是5
    */
    Ok((end, s.parse()?))
}
